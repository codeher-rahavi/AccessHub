...
const SignUp = () => {
    ...
    const handleSignUp = async (e) => {
        ...
        if (!email || emailError || !password || !repeatPassword) return;
        ...
        try {
            ...
            setPasswordError(data.message || "Signup failed. Please try again.");
        } catch (error) {
                ...
                <div className="z-10 absolute -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2 border border-2 border-gray-200 p-7 rounded-2xl shadow-2xl bg-gray-50">
                    <div className="flex flex-col gap-4 w-[450px]">
                        ...
                        <div>
                            ...
                            <input type="email" placeholder="Enter your email" value={email} onChange={handleEmailChange} className={`focus:outline-gray-400 border w-full h-10 p-3 rounded-lg ${emailError ? "border-red-500 focus:outline-red-500" : "border-gray-400"}`} />
                            {emailError && <p className="text-red-500 text-sm mt-1">{emailError}</p>}
                        </div>

                        <div>
                            <PassWord value={password} onChange={handlePasswordChange} />
                            {passwordStrength && <p className={`text-sm mt-1 font-medium ${getStrengthColor()}`}>Password Strength: {passwordStrength}</p>}
                        </div>

                        <div>
                            <RepeatPassWord value={repeatPassword} onChange={(e) => setRepeatPassword(e.target.value)} />
                            {passwordError && <p className="text-red-500 text-sm mt-1">{passwordError}</p>}
                        </div>

                        <button onClick={handleSignUp} disabled={loading} className={`inline-block rounded-lg border border-current px-8 py-3 text-[18px] text-white transition text-center focus:outline-2 focus:outline-green-900 ${loading ? "bg-gray-500 cursor-not-allowed" : "bg-green-700 hover:bg-green-800"}`}>
